...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const seen = new Set(
    JSON.parse(localStorage.getItem("ctuos-phishing-modules") || "[]"),
  );
  ...
  const updateMods = () => {
    ...
    localStorage.setItem("ctuos-phishing-modules", JSON.stringify([...seen]));
  };
  updateMods();
  ...
  document.querySelectorAll(".safecheck").forEach((c) => {
    const k = "ctuos-phishing-check-" + c.dataset.key;
